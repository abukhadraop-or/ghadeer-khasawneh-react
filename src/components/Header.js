import React from "react";
import {
  StyledHeader,
  Styledlist,
  StyledItem,
  StyledIcons,
} from "../styles/Header.Styled";

const Header = () => {
  return (
    <StyledHeader>
      <Styledlist className="list">
        <img
          src="https://www.themoviedb.org/assets/2/v4/logos/v2/blue_short-8e7b30f73a4020692ccca9c88bafe5dcb6f8a62a4c6bc55cd9ba82bb2cd95f6c.svg"
          alt="Logo"
        />

        <StyledItem className="items">Movies</StyledItem>
        <StyledItem className="items">TV Shows</StyledItem>
        <StyledItem className="items">People</StyledItem>
        <StyledItem className="items">More</StyledItem>
      </Styledlist>
      <StyledIcons>
        <div>
          <img
            src="https://www.themoviedb.org/assets/2/v4/glyphicons/basic/glyphicons-basic-371-plus-white-0bac34f16124808a12ea863b4d9cc6e599dee7c0a80658cfe9ead26939e64517.svg"
            alt=""
            width="20px"
          />
        </div>
        <section>EN</section>
        <div>
          <h1>Login</h1>
        </div>
        <div>
          <h1>Join TMDB</h1>
        </div>
        <div>
          <img
            src="https://www.themoviedb.org/assets/2/v4/glyphicons/basic/glyphicons-basic-28-search-blue-177462d06db81ff2a02aa022c1c0be5ba4200d7bd3f51091ed9298980e3a26a1.svg"
            alt="Search Icon"
            width="30px"
          />
        </div>
      </StyledIcons>
    </StyledHeader>
  );
};

export default Header;
